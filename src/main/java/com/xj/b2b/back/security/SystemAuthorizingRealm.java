...
import com.xj.b2b.back.service.SystemService;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		if (user != null) {
			...
			if (User.DEL_FLAG_DELETE.equals(user.getDelFlag())) {
				throw new AuthenticationException("msg:该帐号已删除，禁止登录.");
			}
			
			byte[] salt = Encodes.decodeHex(user.getPassword().substring(0, SystemService.SALT_SIZE * 2));
			return new SimpleAuthenticationInfo(new Principal(user, token.isMobileLogin()),
					user.getPassword().substring(SystemService.SALT_SIZE * 2), ByteSource.Util.bytes(salt), getName());
		} else {
			return null;
		}
	}
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		if (user != null) {
			...
			// 记录系统鉴权日志
			LogUtils.saveLog(Servlets.getRequest(), "系统鉴权");
			return info;
		} else {
			return null;
		}
