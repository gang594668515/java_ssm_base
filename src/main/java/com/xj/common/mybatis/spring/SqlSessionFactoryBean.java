 ...
 */
package com.xj.common.mybatis.spring;

import com.xj.common.mybatis.thread.MapperRefresh;
 ...
 * @see #setDataSource
 *
 * @modify yangb 20171218
 *         重写mybatis-spring-1.3.1的org.mybatis.spring.SqlSessionFactoryBean<br>
 *         1、修改实体类重名的时候抛出并打印异常，否则系统会一直递归造成无法启动。<br>
 *         2、加入启动MapperRefresh.java线程服务。
 */
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		if (hasLength(this.typeAliasesPackage)) {
			...
			for (String packageToScan : typeAliasPackageArray) {
				// 20171218begin 修改实体类重名的时候抛出并打印异常，否则系统会一直递归造成无法启动
				try {
		...
		if (!isEmpty(this.mapperLocations)) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("MapperLocation's length:" + this.mapperLocations.length);
			}
			// 20171218 启动刷新MapperXML定时器（有助于开发者调试）
