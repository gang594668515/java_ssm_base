...
<html>
<head>
<title>${fns:getConfig('productName')}后台管理</title>
<meta name="decorator" content="back_blank" />
<c:set var="tabmode" value="${empty cookie.tabmode.value ? '0' : cookie.tabmode.value}" />
<c:if test="${tabmode eq '1'}">
	<link rel="Stylesheet" href="${ctxStatic}/back/jerichotab/dist/css/jquery.jerichotab.css" />
	<script type="text/javascript" src="${ctxStatic}/back/jerichotab/dist/js/jquery.jerichotab.js"></script>
</c:if>
<style type="text/css">
#main {
	padding: 0;
	...
	function checkNewMsg() {
		$.ajax({
			...
			success: function(data) {
				$("#notifyNum").text(data.newMsg);
				$("#notifyNum2").text(data.newMsg);
			},
...
<body>
	<div id="main">
		<div id="header" class="navbar navbar-fixed-top">
			<div class="navbar-inner">
				<div class="brand">
					<span id="productName">${fns:getConfig('productName')}后台管理</span>
				</div>
				<ul id="userControl" class="nav pull-right">
					<li><a href="${pageContext.request.contextPath}/"
						target="_blank" title="访问网站主页"><i class="icon-home"></i></a></li>
					<li id="themeSwitch" class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" href="#" title="主题切换"><i
						...
						</ul> <!--[if lte IE 6]><script type="text/javascript">$('#themeSwitch').hide();</script><![endif]--></li>
					<li id="userInfo" class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" href="#" title="个人信息">您好,
							${fns:getUser().name}&nbsp;<span id="notifyNum" class="label label-info">0</span>
					</a>
						<ul class="dropdown-menu">
							<li><a href="${ctx}/sys/user/info" target="mainFrame"><i class="icon-user"></i>&nbsp; 个人信息</a></li>
							<li><a href="${ctx}/sys/user/modifyPwd" target="mainFrame"><i class="icon-lock"></i>&nbsp; 修改密码</a></li>
							<li><a href="${ctx}/sys/user/message" target="mainFrame"><i class="icon-envelope"></i>&nbsp; 系统消息(<i id="notifyNum2" style="color: #45aeea;font-weight: bold;font-style: normal;">0</i>)</a></li>
						</ul></li>
					...
					<ul id="menu" class="nav" style="*white-space: nowrap; float: none;">
						...
						<c:forEach items="${fns:getMenuList()}" var="menu" varStatus="idxStatus">
							<c:if test="${menu.parent.id eq '1'&&menu.isShow eq '1'}">
								<li class="menu ${not empty firstMenu && firstMenu ? ' active' : ''}"><c:if test="${empty menu.href}">
										...
										<a class="menu" href="${fn:indexOf(menu.href, '://') eq -1 ? ctx : ''}${menu.href}" data-id="${menu.id}"
											target="mainFrame"><span>${menu.name}</span></a>
									</c:if></li>
								<c:if test="${firstMenu}">
		...
		<div class="container-fluid">
			...
			<div id="footer" class="row-fluid">
				${fns:getConfig('copyright')} - Powered By <a href="${fns:getConfig('poweredByUrl')}" target="_blank">${fns:getConfig('poweredBy')}</a>
	...
	<script src="${ctxStatic}/custom/js/b2b-back/include/frame-resize.min.js" type="text/javascript"></script>
</body>
</html>